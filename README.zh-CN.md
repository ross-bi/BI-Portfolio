...
拥有 20 年运营实务经验的 BI & 运营协调员，专长涵盖船务操作、进出口单证处理、仓库库存追踪
及业务流程协调，现正转型至 Operations Analyst 及 BI Analyst 职位。

丰富的运营背景为我的数据工作带来实质优势：我了解库存如何在供应链中流动、货运单证如何对应
成本中心，以及仓库 KPI（库存周转率、滞销库存、补货点）背后所反映的真实业务决策——而不仅仅
是 Dashboard 上的数字。

我使用 SQL、PostgreSQL、Power BI、dbt 及 BigQuery 构建端到端的 BI Portfolio 项目，分析
销售业绩、客户旅程、广告投资回报及库存运营。我的目标是将业务领域知识与数据工程相结合——
将杂乱的运营数据转化为清晰、可付诸行动的业务洞察。
